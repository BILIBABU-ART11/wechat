...
1. 用户在“我的”页面点击“订阅提醒”。
2. 小程序调用 `wx.requestSubscribeMessage`。
3. 只有微信返回 `accept` 的服务器允许模板 ID 才会计入可用次数。
4. 同一个短期 `request_id` 只能提交一次。
5. 新数据批次到达后，系统只选择以下用户：
   - 院院通用户 ID 与快照中的 `userId` 完全一致；
   - `pendingCount > 0`；
   - 对当前模板仍有可用订阅次数。
6. 消息发送成功后扣减一次可用次数。
7. 消息通常显示在微信“服务通知”中，点击后进入小程序首页。

微信当前使用一次性订阅消息。用户每授权一次通常只能发送一次提醒，发送成功后需要用户再次进入“我的”页面授权。

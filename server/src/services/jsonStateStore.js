...
function hasCosCredentials() {
  return Boolean(config.cos.secretId && config.cos.secretKey);
}

function isConfigured() {
  if (config.storage.mode !== 'cos-json') return false;
  if (config.cos.stateFile) return true;
  return Boolean(config.cos.bucket && config.cos.region && config.cos.stateKey);
}
...
function getCosClient() {
  ...
  if (!hasCosCredentials()) {
    throw new Error('COS credentials are not configured. Set COS_SECRET_ID and COS_SECRET_KEY, or provide TencentCloud runtime credential environment variables.');
  }
  const COS = require('cos-nodejs-sdk-v5');
  cosClient = new COS({
    SecretId: config.cos.secretId,
    SecretKey: config.cos.secretKey,
    SecurityToken: config.cos.sessionToken || undefined
  });
  return cosClient;
}

function cosGetObject() {
  return new Promise((resolve, reject) => {
    getCosClient().getObject({
      Bucket: config.cos.bucket,
      Region: config.cos.region,
      Key: config.cos.stateKey
    }, (error, data) => {
      if (error) {
      ...
      resolve(data && data.Body ? data.Body.toString('utf8') : '');
    });
  });
}

function cosPutObject(body) {
  return new Promise((resolve, reject) => {
...
async function readStateText() {
  if (config.cos.stateFile) {
    try {
      return await fs.readFile(config.cos.stateFile, 'utf8');
    } catch (error) {
...
async function writeStateText(text) {
  if (config.cos.stateFile) {
    await fs.mkdir(path.dirname(config.cos.stateFile), { recursive: true });
    await fs.writeFile(config.cos.stateFile, text, 'utf8');
    return;
  }
  await cosPutObject(text);
}
...
async function saveState(state) {
  ...
  await writeStateText(`${JSON.stringify(normalized, null, 2)}\n`);
  return normalized;
}
